	    ...
	    public String addpaper(@Valid RapportModel rapportModel, BindingResult bindingResult, Model model) throws IllegalStateException, IOException, MessagingException {
		 if (bindingResult.hasErrors()) {
	        ...
	        if(!(rapportModel.getFile().isEmpty())){ 
	        	...
	        	smtpMailSender.send(p.getAuteur().getEmail(), "Acceptation du papier "+p.getTitre() +" soumis a la conférence "+p.getConference().getTitre(), "nous vous informons votre papiers soumis au conference "+p.getConference().getTitre()+ " a été bien accepté , et nous vous invitons a envoyer la version finale");
	        	}
	        	if(rap.getRapportType()==RapportType.REJETE){
		        	smtpMailSender.send(p.getAuteur().getEmail(), "Rejet du papier :"+p.getTitre() +"soumis a la conférence "+p.getConference().getTitre(), "nous vous informons votre papiers soumis au conference "+p.getConference().getTitre()+ " a été refusé , nous sommes vraiment désolé !!! , ");
		        }
